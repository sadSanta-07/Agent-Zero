...
export function TelemetryPanel({
  ...
  return (
    <aside className="w-[340px] border-l border-brand-border bg-brand-surface p-6 flex flex-col justify-between shrink-0 h-full overflow-hidden">
      <div className="flex flex-col h-full min-h-0 gap-4">
        ...
        <div className="flex justify-between items-center border-b border-brand-border pb-3 shrink-0">
          ...
          <span className="text-[10px] font-space-mono font-bold text-brand-primary bg-brand-primary/10 py-1 px-2 rounded-[2px] tracking-wider uppercase">
            System Online
          </span>
        </div>
        ...
        {/* The Log Terminal - Shifted from dark mode to a clean, academic ledger style */}
        <div className="flex-1 bg-brand-bg border border-brand-border text-brand-text-primary font-space-mono text-[12px] p-4 rounded-[3px] overflow-y-auto flex flex-col gap-3 scrollbar-thin min-h-0 shadow-sm">
          {logs.map((log) => {
            // Checking both the old and new terminology to ensure nothing breaks
              ...
              <div key={log.id} className="flex flex-col gap-1 leading-relaxed text-left border-b border-brand-border/60 pb-3 last:border-0 last:pb-0">
                <div className="flex items-center justify-between font-medium text-[10px] text-brand-text-secondary">
                  <span className={`uppercase tracking-wider font-bold ${
                    ...
                    log.agentName === 'Triage Agent' ? 'text-[#9a5b00]' : // Using your accent-badge-text
                ...
                <p className={`select-all break-words m-0 ${isCritical ? "text-red-700 font-medium" : "text-brand-text-primary"}`}>
                  {log.message}
                </p>
              </div>
            );
        ...
        <div className="border-t border-brand-border pt-4 grid grid-cols-3 gap-3 shrink-0 text-center text-[10px] font-space-mono font-bold text-brand-text-secondary">
          ...
          <div className="bg-brand-bg border border-brand-border py-2 px-1 rounded-[2px] flex flex-col items-center transition-all-custom hover:border-brand-text-secondary shadow-sm">
